...
/**
 * Reconciles the stored answer ID with the full option text for display
 */
function getOptionFullText(options: string[] | null, answerId: string): string {
  if (!options || !answerId) return answerId;
  const option = options.find(opt => opt.startsWith(`${answerId},`));
  ...
  return (
    <div className={`card border-l-4 ${isCorrect ? 'border-l-green-500' : 'border-l-red-500'}`}>
      ...
      <div className="grid grid-cols-1 gap-4 text-sm mt-4">
        <div className="space-y-1">
          ...
          <div className={`p-3 rounded bg-slate-950/50 border ${isCorrect ? 'border-green-900/50' : 'border-red-900/50'}`}>
            <MarkdownContent 
              content={getOptionFullText(question.options, answer?.userAnswer || '')} 
        ...
        {!isCorrect && (
          <div className="space-y-1">
            ...
            <div className="p-3 rounded bg-slate-950/50 border border-green-900/50">
              <MarkdownContent 
                content={getOptionFullText(question.options, question.answer)} 
      ...
      {question.explanation && (
        <div className="space-y-1">
         <MarkdownContent className="text-slate-400 text-xs mt-3 border-t border-slate-700 pt-3" content={question.explanation} />
        </div>
      )}
    </div>
  )
