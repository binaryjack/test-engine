...
}).refine(data => data.configs || data.technologyId || (Array.isArray(data.technologyIds) && data.technologyIds.length > 0), {
  message: 'technologyId, technologyIds or configs is required'
})
