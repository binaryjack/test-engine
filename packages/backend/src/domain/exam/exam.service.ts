...
export interface GenerateInput {
  ...
  configs?: { technologyId: string; level: string; count: number }[]
}

export function generateExam(input: GenerateInput): ExamSessionDto {
  let selected: QuestionDto[] = []
  let primaryTechId: string | undefined
  let primaryLevel: string | undefined

  if (input.configs && input.configs.length > 0) {
    // Handle new mixed configuration format
    primaryTechId = input.configs[0].technologyId
    primaryLevel = input.configs.length > 1 ? 'MIXED' : input.configs[0].level

    for (const conf of input.configs) {
      const qs = listQuestions({ technologyId: conf.technologyId, level: conf.level })
      if (qs.length === 0) continue

      const shuffled = input.seed !== undefined
        ? seededShuffle(qs, input.seed)
        : qs.sort(() => Math.random() - 0.5)

      selected = selected.concat(shuffled.slice(0, Math.min(conf.count, shuffled.length)))
    }
  } else {
    // Handle legacy format (support selecting from a single technology or multiple technologies)
    ...
    if (!input.level) {
      throw Object.assign(new Error('No level specified'), { status: 400 })
    }

    primaryTechId = techIds[0]
  ...
  if (selected.length === 0) {
    throw Object.assign(new Error('No questions found for the given configuration'), { status: 422 })
  }
  ...
  runSql(
    `INSERT INTO exam_sessions (id, userId, technologyId, level, mode, questionIds, startedAt)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [id, input.userId, primaryTechId, primaryLevel, input.mode, JSON.stringify(selected.map(q => q.id)), now]
  )
