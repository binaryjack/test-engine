...
export function getQuestionByPrompt(technologyId: string, level: string, prompt: string): QuestionDto | null {
  const sql = `SELECT * FROM questions WHERE technologyId = ? AND level = ? AND TRIM(prompt) = ? LIMIT 1`
  const row = queryOneSql<Question>(sql, [technologyId, level, prompt.trim()])
  return row ? toDto(row) : null
}
